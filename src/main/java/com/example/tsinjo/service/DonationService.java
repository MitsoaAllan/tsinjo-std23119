...
public class DonationService {
    ...
    public void updatePaymentStatuses() {
        List<Donation> verifyingDonations = donationRepository.findByPaymentVerificationStatus(
                VerificationStatus.VERIFYING
        );

        verifyingDonations.forEach(donation -> {
            Payment updatedStatus = volaService.verifyPayment(
                    donorRepository.findById(donation.getId())
